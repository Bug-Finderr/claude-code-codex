...
Every invocation explicitly disables Claudish auto approval and passes Claude Code's `--dangerously-skip-permissions` flag directly before the passthrough separator. It also disables Claudish usage stats, logs, and diagnostics and skips model-catalog updates. The OpenAI key and official base URL exist only in the Claudish child environment. Inherited Anthropic credentials are removed from that child without changing the parent PowerShell environment.
